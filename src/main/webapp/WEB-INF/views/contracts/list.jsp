                                                            ...
                                                            <c:forEach var="contract" items="${contracts.content}">
                                                                <tr>
                                                                    <td>
                                                                        ...
                                                                        <c:if
                                                                            test="${not empty contract.content and contract.content.length() > 100}">
                                                                            <br>
                                                                    ...
                                                                    <td>
                                                                        <div class="small">
                                                                            <strong>갑:</strong>
                                                                            <c:out
                                                                                value="${contract.firstParty.name}" />
                                                                            <br>
                                                                            <strong>을:</strong>
                                                                            <c:out
                                                                                value="${contract.secondParty.name}" />
                                                                        </div>
                                                                    </td>
                                                                    <td>
                                                                        <c:choose>
                                                                            <c:when
                                                                                test="${contract.status == 'DRAFT'}">
                                                                                <span
                                                                                    class="badge bg-secondary">초안</span>
                                                                            </c:when>
                                                                            <c:when
                                                                                test="${contract.status == 'PENDING'}">
                                                                                <span class="badge bg-warning">서명
                                                                                    대기</span>
                                                                            </c:when>
                                                                            <c:when
                                                                                test="${contract.status == 'SIGNED'}">
                                                                                <span class="badge bg-success">서명
                                                                                    완료</span>
                                                                            </c:when>
                                                                            <c:when
                                                                                test="${contract.status == 'CANCELLED'}">
                                                                                <span class="badge bg-danger">취소</span>
                                                                            </c:when>
                                                                            <c:when
                                                                                test="${contract.status == 'EXPIRED'}">
                                                                    ...
                                                                    <td>
                                                                        <fmt:formatDate value="${contract.createdAt}"
                                                                            pattern="yyyy-MM-dd HH:mm" />
                                                                    </td>
                                                                    <td>
                                                                        <c:choose>
                                                                            <c:when
                                                                                test="${not empty contract.expiresAt}">
                                                                                <fmt:formatDate
                                                                                ...
                                                                                <c:otherwise>
                                                                                    ...
                                                                                    value='${contract.id}'/>')"
                                                                                        ...
                                                                                        <button type="button"
                                                                                            ...
                                                                                            onclick="deleteContract('<c:out
                                                                                    value='${contract.id}'/>', '<c:out
                                                                                    ...
                                                                                    <c:if
                                                                                        test="${contract.status == 'PENDING' or contract.status == 'SIGNED'}">
                                                                                        <button type="button"
                                                                                            class="btn btn-outline-warning"
                                                                                            onclick="cancelContract('<c:out
                                                                                    value='${contract.id}'/>')"
                                                                                            title="취소">
                                                                                            <i
                                                    ...
                                                    <nav aria-label="계약서 목록 페이지네이션" class="mt-4">
                                                        <ul class="pagination justify-content-center">
                                                            <c:if test="${contracts.hasPrevious()}">
                                                                <li class="page-item">
                                                                    <a class="page-link"
                                                                        href="?page=${contracts.number - 1}&status=${param.status}">
                                                                        <i class="bi bi-chevron-left"></i>
                                                                    </a>
                                                                </li>
                                                            </c:if>

                                                            <c:forEach begin="0" end="${contracts.totalPages - 1}"
                                                                var="i">
                                                                <li
                                                                    class="page-item ${i == contracts.number ? 'active' : ''}">
                                                                    <a class="page-link"
                                                                        href="?page=${i}&status=${param.status}">${i +
                                                                        1}</a>
                                                                </li>
                                                            </c:forEach>
